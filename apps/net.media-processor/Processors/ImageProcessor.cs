...
{
  public class ImageProcessor : BaseProcessor, IProcessor
  {
    ...
    public ImageProcessor(IFFMpegService ffMpegService, IImageService imageService, IConfiguration configuration,
      ILogger logger, IMessageClient messageClient, IFileService fileService, IFtpService ftpService,
      IDataContextFactory dbContextFactory): base(configuration, logger, messageClient)
    {
    ...
    public void Run()
    {
      ...
      _logger.Information(
        $"Image Processor subscribes to Exchange '{MessageExchanges.ProcessorExchange}', Queue '{queueName}', bindingKeys '{bindingKeys[0]}'");
      _subscription =
        _messageClient.Subscribe(MessageExchanges.ProcessorExchange, queueName, bindingKeys, HandleMessage);
    }
    ...
    private async Task HandleMessage(Message m)
    {
      ...
      switch (m.Topic)
      {
        case MessageTopics.ImageUploaded:
          task = HandleImageUploadMessage(m);
          break;
        case MessageTopics.ImageMergeAudio:
          task = HandleImageMergeAudioMessage(m);
          break;
        default:
    ...
    private string TransferImageToCloud(ImageDto dto, MessageIdentity messageIdentity)
    {
      try
      {
        ...
        _ftpService.Upload(dto.RawFilePath, _imageFolder + "/" + dto.CreatedBy + "/" + dto.Id, _ftpStreamingAddress,
          messageIdentity);
        try
        {
